...
export function TempInput({ vref, ...otherProps }: Omit<GetProps<typeof Input>, 'value' | 'onChange' | 'onInput' | 'defaultValue'> & {
  ...
  return (<Input {...otherProps}
    defaultValue={vref.value} onChange={({ target: { value } }) => vref.value = value} />);
}
/**非受控的Radio.Group，无需useState */
export function TempRadioGroup<T>({ vref, ...otherProps }: Omit<GetProps<typeof Radio.Group>, 'value' | 'onChange' | 'defaultValue'> & {
  vref: { value: T }
}) {
...
function StateDisabledContextProvider({ stateSetterReceiver, children }: { stateSetterReceiver: (setter: (disabled: boolean) => void) => void, children: ReactNode }) {
 ...
 */
export function showModal({ content, onConfirm, okButtonProps, ...otherProps }: ModalFuncProps & { onConfirm: () => Promise<void | boolean> }): Promise<boolean> {
  return new Promise((rs) => {
    //设置弹窗内部组件的disabled状态（不包括弹窗本身ok按钮）
    let setDisabled: (disabled: boolean) => void;
    const newContent = (<StateDisabledContextProvider stateSetterReceiver={(setState) => setDisabled = setState}>
      {content}
