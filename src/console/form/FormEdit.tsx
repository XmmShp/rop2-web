...
export const builtinPhoneQuestion = {
  type: 'text',
  title: '您的手机号',
  maxLine: 1,
  id: -1,
...
export default function FormEdit() {
  ...
  useEffect(() => {
    ...
    function scrollHandler() {
      ...
      if (curG !== curGroupIndex) setCurGroupIndex(curG);
    }
  ...
  return (
    <Flex className="editor">
      <FloatButton tooltip="预览表单" type="primary" icon={<EyeOutlined />} onClick={() => window.open(`${basename}/apply/${form.id}?preview=1`, '_blank')} />
      ...
      <Flex className="page" vertical ref={pageRef}>
        <Flex className="form" vertical gap="middle">
          <Typography.Text type="secondary">
            ...
            <br />
            候选人正常填表地址：
            <CopyZone inline text={`${location.origin}${basename}/apply/${form.id}`} />
          </Typography.Text>
          <Tooltip
            title={
              <>
          ...
          >
            <Flex vertical>
              ...
              <Typography.Text type="secondary">
                开放时间内候选人才可看到/提交问卷。开放时间内，候选人可重复提交问卷，其所有志愿也会重置到"已填表"阶段。
              </Typography.Text>
      ...
      items={[
        {
          ...
          label: (
            ...
            >
              ...
              <Button disabled={!showMoveUp} type="link" size="small" icon={<ArrowUpOutlined />} onClick={() => onMove(-1)} />
              <Button disabled={!showMoveDown} type="link" size="small" icon={<ArrowDownOutlined />} onClick={() => onMove(1)} />
              <Button disabled={isEntry} type="link" size="small" icon={<DeleteOutlined />} onClick={() => onDelete()} />
            </Flex>
          ),
          children: (
              ...
              {questions.map((ques, index) => (
                <PreviewWithEditor
                  ...
                  onConfirm={async (newObj) =>
                    await onEdit({
                      ...group,
                      children: questions.with(index, newObj),
                    })
                  }
                  onDelete={async () =>
                    await onEdit({
                      ...group,
                      children: questions.toSpliced(index, 1),
                    })
                  }
                  onMove={async (delta) =>
                    await onEdit({
                      ...group,
                      children: moveElement(questions, index, delta),
                    })
                  }
                />
              ))}
              <Flex wrap="wrap" align="center" gap="small">
                <Button
                  ...
                  onClick={async () => {
                    ...
                    await onEdit({
                      ...group,
                      children: [...questions, { type: 'text', title: '新问题', id: maxId + 1 }],
                    });
                  }}
                >
                  新建问题
                </Button>
                <Button icon={<PlusOutlined />} onClick={async () => await onAppendGroup()}>
                  追加问题组
                </Button>
                <Checkbox
                  checked={group.hideSeparator ?? false}
                  onChange={async ({ target: { checked } }) => {
                    await onEdit({ ...group, hideSeparator: checked });
                  }}
                >
                  隐藏组标题和分隔符
                </Checkbox>
