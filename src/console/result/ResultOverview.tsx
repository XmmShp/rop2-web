...
import { downloadBlob, toCsv } from '../../exportUtils';
...
export default function ResultOverview() {
  ...
  const [intentList, loading, reload] = useData<IntentList>(
    '/result/intents',
    async (resp) => {
    ...
    [offset, limit, filter, filterDeparts, form.id, step],
    defaultDepart > 0
  );
    ...
    <Card loading={!!orgInfoLoading}>
      <Flex vertical gap="middle">
        ...
        <Segmented
          ...
          value={step}
          onChange={setStep}
          options={validSteps.map((n) => {
            return {
              label: (
                <>
                  {getStepLabel(n)} {n === step ? `(${count})` : null}
                </>
              ),
              value: n,
            };
      ...
      <Radio.Group defaultValue='independent'>
          ...
          <Space size="small">
            ...
            <Button
              onClick={async () => {
                ...
                type Column = {
                  title: string;
                  format(intent: IntentOutline, resultDetail: ResultDetail): string;
                };
                ...
                const cols: Column[] = [
                  ...
                  },
                  {
                    title: '志愿部门',
                    format(intent) {
                      return formatIntentDepart(intent);
                    },
                  },
                  {
                    title: '面试时间',
                    format(intent) {
                      return formatInterviewTime(intent);
                    },
                  },
                  ...form.children
                    .map((group) =>
                      group.children.map((ques) => {
                        return {
                          ...
                          format(_: never, resultDetail: ResultDetail) {
                            return formatAnswer(ques, resultDetail.content[ques.id], departs, '');
                          },
                        };
                      })
                    )
                ...
                selectedIntents.forEach((curIntent) => {
                ...
                const blob = new Blob(
                  //BOM头 避免Excel打开乱码
                  [new Uint8Array([0xef, 0xbb, 0xbf]).buffer, csvText],
            ...
            <Dropdown.Button
              type="primary"
              onClick={() => {
                setIntentsStep(selectedKeys, getNextStep(step));
              }}
              menu={{
                items: [
                  {
                    label: (
                      <Button onClick={() => setIntentsStep(selectedKeys, accepted)} size="small" type="link">
                        直接录取
                      </Button>
                    ),
                  },
            ...
            <Dropdown.Button
              danger
              onClick={() => {
                setIntentsStep(selectedKeys, rejected);
              }}
              menu={{
                items: [
                  {
                    label: (
                      <Button
                        onClick={() => {
                          setIntentsStep(selectedKeys, invalid);
                        }}
                        type="link"
                        size="small"
                        danger
                  ...
                  },
                  {
                    label: (
                      <Button
                        onClick={() => {
                          setIntentsStep(selectedKeys, initial);
                        }}
                        type="link"
                        size="small"
                        danger
            ...
            <Button
              onClick={() =>
                copyPhones(
                  selectedKeys.map((k) => {
                    const intent = intents.find((i) => i.id === k);
                    if (copyPhonesWithoutInterviewOnly && hasInterview && intent?.interviewTime) return null;
                    return intent?.phone;
                  })
                )
              }
        ...
        {filterDeparts.length > 1 && <Alert showIcon type="warning" message={`您正在查看来自多个部门的候选人(已选中 ${filterDeparts.length} 个部门)。`} />}
                    ...
                    {step >= 0 && (
                      <>
                        <Dropdown.Button
                          ...
                          type="link"
                          onClick={() => {
                            setIntentsStep([record.id], getNextStep(step));
                          }}
                          menu={{
                            items: [
                              {
                                label: (
                                  <Button
                                    onClick={() => {
                                      setIntentsStep([record.id], accepted);
                                    }}
                                    size="small"
                                    type="link"
                                  >
                        ...
                        <Dropdown.Button
                          ...
                          type="link"
                          onClick={() => {
                            setIntentsStep([record.id], rejected);
                          }}
                          menu={{
                            items: [
                              {
                                label: (
                                  <Button
                                    onClick={() => {
                                      setIntentsStep([record.id], invalid);
                                    }}
                                    size="small"
                                    type="link"
                                    danger
                              ...
                              },
                              {
                                label: (
                                  <Button
                                    onClick={() => {
                                      setIntentsStep([record.id], initial);
                                    }}
                                    size="small"
                                    type="link"
                                    danger
          ...
          title={(d) => (
            <Space size="small">
              <Typography.Text
                strong
                style={{
                  color: step === 0 ? 'gray' : step > 0 ? '#00b8ff' : step === -50 ? 'green' : 'red',
                }}
              >
                {getStepLabel(step)}
              </Typography.Text>
              候选人列表 (本页 {d.length} 项{filter ? ` / 筛选到 ${filteredCount} 项` : ''} / 共 {count} 项)
            </Space>
          )}
        />
      </Flex>
