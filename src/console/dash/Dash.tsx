...
function countdownText(now: Dayjs, futureMoment: Dayjs): string {
  ...
  if (daysLeft <= 0) return `${padZero(secondsLeft / 3600)}:${padZero((secondsLeft % 3600) / 60)}:${padZero(secondsLeft % 60)}`;
...
function CountdownStatistic({ startAt, endAt }: { startAt: Dayjs | null; endAt: Dayjs | null }) {
  const now = dayjs();
  if (startAt && startAt.isAfter(now))
    //如果startAt不存在，等同已开始
    return <Statistic title="距离报名开始" value={countdownText(now, startAt)} prefix={<HourglassOutlined />} />;
  if (endAt)
    if (endAt?.isAfter(now)) return <Statistic title="距离报名结束" value={countdownText(now, endAt)} prefix={<HourglassOutlined />} />;
    else return <Statistic title="报名状态" value="已结束" />;
}
export default function Dash() {
  ...
  const [{ steps, peopleCount }, statisticLoading] = useData<StepStatistics>(
    '/form/statistic',
    async (resp) => {
      ...
      {statisticLoading ? (
        ...
        <>
          ...
          <Flex wrap="wrap" gap="small" align="flex-start">
            {steps.map(({ id, peopleCount, intentsCount, interviewDone, interviewCount }) => (
              <Card key={id}>
                <Flex vertical gap="small">
                  <Flex wrap="wrap" gap="large">
                    <Statistic title="阶段" value={getStepLabel(id)} valueStyle={{ color: getStepColor(id) }} />
                    <Statistic title="人数" value={peopleCount} prefix={<TeamOutlined />} />
                    <Statistic title="志愿数" value={intentsCount} />
                  </Flex>
                  {interviewCount == 0 && !stepsWithInterview.some((v) => v === id) ? (
                    <></> //目前没有安排面试的阶段不显示面试信息
                  ) : (
                    <Flex wrap="wrap" gap="large">
