...
export function logout(callbackUrl: string = location.href) {
  ...
  location.href = `https://zjuam.zju.edu.cn/cas/logout?service=${encodeURIComponent(`https://www.qsc.zju.edu.cn/passport/v4/logout?success=${encodeURIComponent(callbackUrl)}`)}`;
}
...
export function getLoginRedirectUrl() {
  ...
  const redirectUrl = `https://www.qsc.zju.edu.cn/passport/v4/zju/login?success=${encodeURIComponent(loginCallbackUrl)}`;
  return redirectUrl;
}
