...
export default function ApplyForm() {
  ...
  const [{ owner, name, desc, children }, formLoading] = useData<FormDetail>(isPreview ? '/form/detail' : '/applicant/form', async (resp) => {
    if (resp.status === 404) return { ...defaultForm, id: formId, children: { message: '未找到表单(404)' } };
  ...
  }, { ...defaultForm, id: formId }, { id: formId }, [formId], formId > 0);
  useEffect(() => {
    if (name)
      document.title = `${name} - 纳新开放系统`;
  }, [name])
          ...
          {'message' in children
            ...
            (completed
              ...
              <Flex vertical gap='large'>
                {formLoading ? <Skeleton active loading />
                  : <>
                    <Typography.Title level={3} className='title'>
                      {name}
                    </Typography.Title>
                    {desc.length > 0 && <Typography.Text>
                      {desc}
                    </Typography.Text>}
                    <Form layout='vertical'
                      onFinish={async (v) => {
                        const choiceDepartQuestion = children.first(
                          (g) => g.children.first(
                            ...
                            <FormQuestion
                              ...
                              onChange={(v) => {
                                ...
                                if (ques.type === 'choice' || ques.type === 'choice-depart')
                                  setRevealGroups(calcRevealGroups(children, newAnswer));
                              }}
                              departs={departs} />
                          ))}
                        </Flex>)
  ...
  return (<>
    <Result
      ...
      subTitle={<div className='success-subtitle'>
        感谢您参与{name}。
        <br />
        如需<a href={`${basename}/apply/${formId}`}>修改答卷</a>，您可在问卷开放时间内返回此页，重新提交答卷。
      </div>}
    />
